...
def receive_neg_partition(recv_num,file,gene,r_child_conn,parent_conn):
    recv_list=[]
    for i in range(recv_num):
        while 1:
            try:
...
def parallel_neg_main(file,gene,gene_readindex_rows,needed_dict,avoid_poses,task_queue_1,task_queue_2,child_conn,locks):    
    ...
    receiver_2=multiprocessing.Process(target=receive_neg_partition,args=(recv_num_2,file,gene,r_child_conn_2,parent_conn_2))
    receiver_2.start()
...
def receive_neg_main(recv_num,file,needed_dict,r_child_conn,parent_conn):
    LABEL=0
    for i in range(recv_num):
        while 1:
            try:
                ...
                if partition_data is not None:
                    if select_info[0] in needed_dict:
                        ...
                        with open(file+'_processed_neg.json','a') as fw_json:
                            with open(file+'_processed_neg.index','a') as fw_json_index:
                                ...
                                fw_json_index.write('%s_%s_%d\t%d\t%d\t%d\n'%(gene,select_info[0],select_info[1],file_pos_start,file_pos_end,LABEL))
                break
            except EOFError:
                sleep(0.01)
    r_child_conn.send(needed_dict)

def parallel_process_negative(file,n_processes_0,n_processes_1,n_processes_2):
    lock_0=dict()
    ...
    open(file+'_processed_neg.json','w')
    open(file+'_processed_neg.index','w')
    ...
    needed_dict={}
    avoid_dict={}
    with open(file+'_processed.index','r') as f:
        lines=f.readlines()
        for line in lines:
            sinfo=line.split()[0]
    ...
    while 1:
        ...
        while 1:
            ...
            actual_recv_num+=1
            if actual_recv_num==RECV_NUM or i==len(genes):
                break
        t_parent_conn,t_child_conn=multiprocessing.Pipe()
        t_receiver=multiprocessing.Process(target=receive_neg_main,args=(actual_recv_num,file,needed_dict,t_child_conn,parent_conn_0))
        t_receiver.start()
...
if __name__ == '__main__':
    ...
    for nano_file in ['../events/SGNex_Hct116_directRNA_replicate3_run4-ref.eventalign','../events/SGNex_Hct116_directRNA_replicate4_run3-ref.eventalign','../events/SGNex_Hct116_directRNA_replicate3_run1-ref.eventalign']:
        parallel_process_negative(nano_file,N_processes_0,N_processes_1,N_processes_2)
